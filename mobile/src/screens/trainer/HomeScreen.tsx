...
export default function TrainerHomeScreen() {
  ...
  return (
    ...
    >
      ...
      {/* Check-in status */}
      <View style={[styles.card, { borderLeftColor: data?.isCheckedInToday ? colors.success : colors.danger }]}>
        <Text style={styles.cardTitle}>Today's Attendance</Text>
        <Text style={[styles.cardSub, { color: data?.isCheckedInToday ? colors.success : colors.danger }]}>
...
const styles = StyleSheet.create({
  ...
  statNum: { color: colors.primary, fontSize: 28, fontWeight: '800', marginBottom: 4 },
  ...
  cardTitle: { color: colors.text, fontSize: 13, fontWeight: '600', marginBottom: 4 },
  cardSub: { color: colors.textSecondary, fontSize: 13 },
  sessionMember: { color: colors.text, fontSize: 16, fontWeight: '700', marginBottom: 4 },
});
